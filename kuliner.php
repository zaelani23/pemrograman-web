...
?>
<title>Wisata Kuliner Jogja oleh Dinas Pariwisata DIY</title>
<?php
  include("navbar.php");
  $query = "SELECT * FROM posts ps 
      WHERE published=true AND ps.id IN 
      (SELECT pt.post_id FROM post_topic pt 
        WHERE pt.topic_id=10 GROUP BY pt.post_id) GROUP BY ps.id DESC";
...
<div class="row p-4">
      <div class="container">
        <div class="col">          
            <p class="text-muted">Home > Kuliner</p>
        </div>
        <hr>
      </div>
</div>
    ...
    <div class="row">
      <div class="col-md-12 text-center">
        <h1 class="py-4">Kuliner</h1>
         <h2 style="font-size: 30px;font-family:Oleo Script;font-weight:400;" class="vc_custom_heading text-center font-italic mb-5">Dinas Pariwisata DIY – Informasi Seputar Wisata Kuliner Jogja yang menghadirkan cita rasa khas Yogyakarta.</h2>
        </div>
      </div>
    </div>
  ...
  while ($tampil = mysqli_fetch_assoc($result)) {
        ...
        $kutipan = substr(html_entity_decode($tampil['body']), 380, 400);
...
?>
          ...
          <div class="media mt-3">
            ...
            <div class="media-body">
              ...
              <small class="text-muted"><small class="bg-dark text-white font-weight-bold">&nbsp;KULINER&nbsp;</small>&nbsp;<strong>ADMIN PROGRAM</strong>&nbsp;- <?php echo $tanggal; ?> &nbsp;&nbsp;<i class="fas fa-comments"></i>&nbsp;0</small><br>
